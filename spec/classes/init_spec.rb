...
describe 'ssh' do
  ...
  context 'with default params on osfamily Solaris operatingsystem Solaris' do
    let :facts do
      {
        :fqdn            => 'monkey.example.com',
        :osfamily        => 'Solaris',
    ...
    it 'should fail' do
      expect {
        should include_class('ssh')
      }.to raise_error(Puppet::Error,/ssh supports Solaris variant Solaris. Your osfamily is <Solaris> and operatingsystem is <Solaris>./)
    end
  end
  ...
  context 'with default params on osfamily Solaris operatingsystem Solaris' do
    ...
    it { should contain_file('sshd_config').with_content(/^ChallengeResponseAuthentication no$/) }
    it { should contain_file('sshd_config').with_content(/^PrintMotd yes$/) }
    it { should contain_file('sshd_config').with_content(/^Banner \/etc\/issue$/) }
